...
export default function RepoDetail() {
  ...
  const kpis = useMemo(() => {
    ...
    const avgDupPerc = totalRuns > 0
      ...
      : 0;
    const lastUnused = last?.metrics?.knip?.findings?.length ?? (last?.metrics?.tsPrune?.count ?? 0);
    const lastArch = last?.metrics?.architecture?.findings?.length ?? 0;
    const lastGeneratedAt = last?.generatedAt || reports?.generatedAt;
    return { totalRuns, lastIssues, lastErrors, lastWarnings, deltaIssues, avgIssues, avgDupPerc, lastUnused, lastArch, lastGeneratedAt };
  }, [historyList, reports]);
              ...
              <Card className="border-border/50 h-auto lg:h-[71vh] flex flex-col overflow-hidden">
                ...
                <CardContent className="flex-1 flex flex-col min-h-0 lg:overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                    ...
                    <div className="rounded-md border border-border/50 p-3 bg-card/30">
                      <p className="text-xs text-muted-foreground">Knip (Unused)</p>
                      <p className="text-base font-semibold">{kpis.lastUnused}</p>
                    </div>
                    <div className="rounded-md border border-border/50 p-3 bg-card/30">
                      <p className="text-xs text-muted-foreground">Arch Issues</p>
                      <p className="text-base font-semibold">{kpis.lastArch}</p>
                    </div>
                    <div className="rounded-md border border-border/50 p-3 bg-card/30">
                                  ...
                                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs text-muted-foreground">
                                    ...
                                    {run.metrics && (
                                      <>
                                        ...
                                        {run.metrics.knip?.findings && (
                                          <span className="text-purple-400">
                                            K: {run.metrics.knip.findings.length}
                                          </span>
                                        )}
                                        {run.metrics.architecture?.findings && (
                                          <span className="text-orange-400">
                                            A: {run.metrics.architecture.findings.length}
                                          </span>
                                        )}
                                        {typeof delta === 'number' && (
