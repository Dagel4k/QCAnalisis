    ...
    renderHtml(model) {
                            ...
                            <meta name="viewport" content="width=device-width, initial-scale=1.0">
                                ...
                                <style>
                                    ...
                                    * {margin:0; padding:0; box-sizing:border-box; }
                                    ...
                                    html.light .code-line .lc { color: #2d3748; }
                                    html.light .ln { color: #a0aec0; }
                                    html.light .snippet { background: #edf2f7; border: 1px solid var(--border); }
                            ...
                            <body>
                                ...
                                <header>
                                    ...
                                    <div class="header-controls">
                                        ...
                                        <button class="btn-icon" onclick="window.print()" aria-label="Print PDF" title="Print to PDF"><svg class="icon"><use href="#icon-print"/></svg><span class="btn-text">PDF</span></button>
                                        <div style="height:20px; border-left:1px solid var(--border); margin:0 4px;"></div>
        ...
        Array.from(allTools).sort().forEach(tool => {
            ...
            chip.onclick = () => selectTool(tool);
                                    ...
                                    function toggleView() {
                                        document.getElementById('summary-view').classList.toggle('active');
                                        document.getElementById('center-panel').querySelector('.detail-view')?.classList.toggle('hidden');
                                    ...
                                    function exportToCSV() {
                                        ...
                                        const headers = ['File', 'Tool', 'Rule', 'Severity', 'Line', 'Message'];
                                        const csvContent = [
                                            headers.join(','),
                                            ...issues.map(i => {
                                                const row = [
                                    ...
                                    function render() {
                                        ...
                                        if (emptyState) {
                                            if (issues.length === 0 && !currentFilter.fileId && !currentFilter.tool && !currentFilter.search) {
                                                emptyState.classList.remove('hidden');
                                                detailView.classList.add('hidden');
                                            } else {
                                                emptyState.classList.add('hidden');
                                                if (!summaryViewActive) detailView.classList.remove('hidden');
                                            }
                                        }
